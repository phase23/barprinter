...
public class LoyaltyFragment extends PreferenceFragment {
    ...
    private myCardHandler mHandler;
    ...
    String title = "Loyalty Points";
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        //setHasOptionsMenu(true);
        addPreferencesFromResource(R.xml.pref_loyalty);
        //mHandler = new LoyaltyFragment.CardHandler(this);
        mHandler = new myCardHandler(this);
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        findPreference(getString(R.string.key_registercard)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
                ...
                //switchFragment(LoyaltyFragment.this, new CardFragment());
                isMF_Plus = false;
...
*/
                ...
                switch (cardTypeNew) {
                    case RF_CARD:
                        ...
                        if (endCardType == SdkData.RF_TYPE_A) {
                            isShowDialog = false;
                            if (!isMF_Plus) {
                                readM1Card2(mCardType, READ_TIMEOUT, msg);    //M1卡
                            } else {
    ...
    private void readRfCard(final CardReaderTypeEnum cardType, byte rfCardType, final int timeout, Message msg) {
        ...
        if (resetStatus != SdkResult.SDK_OK) {
            ...
            if (rfCardType == SdkData.RF_TYPE_FELICA) { // felica card
                ...
                Log.i("[print]","rfCardSend APDU_SEND_FELICA card is RF_TYPE_FELICA");
            } else {
                rfCardSend(msg, rfCard, APDU_SEND_RF, rfCardType);
                Log.i("[print]","rfCardSend APDU_SEND_RF");
            }
        }
        try {
            Thread.sleep(150);
        ...
        if (ifSearch && cardType == CardReaderTypeEnum.RF_CARD) {
            ...
            while (iscontinue) {
                ...
                if (i == SdkResult.SDK_RF_ERR_NOCARD) {
                    iscontinue = false;
                    Log.i("[print]","contine is false");
                }
            }
            if (ifSearch) {
                mCardReadManager.searchCard(cardType, timeout, endCardType, mListener);
                Log.i("[print]","search another card");
            }
        }
    }
    ...
    private void readM1Card2(CardReaderTypeEnum cardType, int timeout, Message msg) {
...
*/
        ...
        int status1 = rfCard.m1VerifyKey((byte) (4 * 0), keyType, key);
        if (status1 == SdkResult.SDK_OK) {
            m1_message += "";
            for (int i = 0; i < 1; i++) {
                byte[] out = new byte[16];
                rfCard.m1ReadBlock((byte) (4 * 0 + i), out);
                m1_message += com.zcs.sdk.util.StringUtils.convertBytesToHex(out);
            }

          //  Log.d("myacci : to /",  StringUtils.convertBytesToHex(out));
...
/*
    ...
    class myCardHandler extends Handler implements DialogInterface.OnClickListener, DialogInterface.OnCancelListener {
        WeakReference<Fragment> mFragment;
        int code = 9999;


        myCardHandler(Fragment fragment) {
